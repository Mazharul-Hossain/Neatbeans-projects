  ...
  // ... (insert Node class definition here)
    ...
    {					// node in the NodeList
        ...
        Node ( const Object& e = Object(),
             int c = WHITE, int m = 0, Node* p = NULL,
             Node* n = NULL )
                : element(e), colour(c),
                member(m), prev(p), next(n)
                {
                    for(int i=0; i<100; i++)
                    {
                        Adj[i] = 0;
...
public:
   ...
   Position insertBefore(const Position& p, const Object& element)
    ...
    void setColour( const Position& p, int c )
        //throw(InvalidPositionException)
    {			// set Colour
        NodePtr v  = nodePtr(p);
        v->colour = c;
    ...
    void setAdjacent( const Position& p, int c )
        //throw(InvalidPositionException)
    {			// set Colour
        NodePtr v  = nodePtr(p);
        v->Adj[c] = 1;
    ...
    int getAdjacent( const Position& p )
    {
    }
